...
3. Confirm model/effort in `Main\MODEL_EFFORT_ASSIGNMENTS.md`.
4. Read `Main\SELF_HOSTED_LLM_POLICY.md` before touching `AI_LLM_Intelligence_Plan` or `AI_Front`.
5. Read the assigned lane file, for example `Lane_1\LANE_INSTRUCTIONS.md`.
6. Load environment variables from `Main\env\codex_lane_env.template.ps1` or copy it to a local untracked lane env file.
